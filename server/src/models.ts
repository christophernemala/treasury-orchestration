...
export interface User { id:string; email:string; name:string; role:Role; passwordHash:string; onboarded:boolean; disabled?:boolean }
export interface Membership { userId:string; tenantId:string; role:Role; legalEntityIds:string[]; active:boolean }
...
export interface StoreState { users:User[]; memberships:Membership[]; transactions:Transaction[]; statements:Statement[]; unapplied:UnappliedCash[]; journals:Journal[]; audit:AuditEvent[]; agentTasks:AgentTask[] }
