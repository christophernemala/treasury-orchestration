...
async function token(){const app=createApp();const login=await request(app).post('/api/auth/login').send({email:'admin@treasury.local',password:'Treasury123!'});const verify=await request(app).post('/api/auth/verify-otp').send({challengeId:login.body.challengeId,otp:login.body.devOtp});return {app,value:verify.body.token}}
describe('treasury API',()=>{
  beforeEach(()=>store.reset());
  it('reports layered runtime readiness without overstating mutation safety',async()=>{const res=await request(createApp()).get('/api/health/runtime');expect(res.status).toBe(200);expect(res.body.status).toBe('propose_only');expect(res.body.canMutate).toBe(false);expect(res.body.layers.governance.status).toBe('propose_only')});
  it('requires authentication for dashboard data',async()=>expect((await request(createApp()).get('/api/dashboard')).status).toBe(401));
  it('returns labeled seeded dashboard data after OTP',async()=>{const x=await token();const res=await request(x.app).get('/api/dashboard').set('Authorization',`Bearer ${x.value}`);expect(res.status).toBe(200);expect(res.body.dataMode).toBe('seeded-development');expect(res.body.reconciliation.total).toBe(7)});
